...
<Callout type="tip" title="Jupyter Notebook 예제">
  **LeRobot 에서 제공하는 Jupyter Notebook을 Colab에서 돌려보기:**
...
export HF_USER="HueyWoo"
```

<Tabs tabs={['기본 설정', '추가 설정']}>
  <div class="tab-panel">
    ```bash
    # ACT 모델 학습 기본 설정
    lerobot-train \
...
기본으로 제공되는 추론 및 실행 코드는 기본적으로 record 코드와 같습니다.

따라서 추론 에피소드를 실행하면 해당 에피소드는 데이터셋 record와 같이 저장됩니다.

만약, 매 에피소드 별로 끊어지고 저장되는게 싫다면 에피소드 타임을 아주 길게 하면 됩니다.
...
<Tabs tabs={['기본 설정', '시간 늘리기']}>
  ...
  <div class="tab-panel">
    ```bash
    # 추론 및 실행 시간 설정을 길게 해서 끊기지 않고 반복 작업 수행
    lerobot-record \
      --robot.type=so101_follower \
      --robot.port=/dev/ttyACM0 \
      --robot.id=follower \
